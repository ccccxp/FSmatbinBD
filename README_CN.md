...
1. **导入配置**：从 FLVER 编辑器导出材质 JSON 文件，并在 **「材质替换」** 功能中导入该文件。
2. **匹配材质**：利用 **「材质匹配」** 功能，根据网格的材质路径或名称，自动匹配当前游戏（目标游戏）中可用的兼容材质。
3. **执行替换**：在 **「材质替换」** → **「纹理编辑」** → **「批量替换材质」** 中填入匹配到的材质名称，执行自动替换。
   - *注*：如果贴图路径留空，工具会自动从数据库中获取标准贴图，并智能匹配到对应的采样器槽位中。
